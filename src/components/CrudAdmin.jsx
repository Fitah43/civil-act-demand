...
const SignUpAdminSchema = Yup.object().shape({
  ...
  role: Yup.string()
    ...
    .oneOf(["admin"], 'Le rôle doit être "admin"'),
});
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:3005/api/admin/create",
        values
      );
      ...
      setAdmins((prevAdmins) => [
        ...prevAdmins,
        {
          ...
          first_name_admin: values.first_name,
        }, // Adaptez cette structure selon la réponse de votre API
      ]);
      // fetchAdmins();
      setIsClick(true);
  ...
  return (
    <>
      ...
      <div style={{ marginTop: "40px", marginLeft: "40px" }}>
        ...
        <AdminList
          isclick={isclick}
          onClick={(isclick) => setIsClick(isclick)}
          admins={admins}
        />
      </div>
    </>
